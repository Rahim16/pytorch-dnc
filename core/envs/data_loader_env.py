...
class DataLoaderEnv(Env):
    def __init__(self, args, env_ind=0):
        super(DataLoaderEnv, self).__init__(args, env_ind)
        ...
        self.train_mask, self.dev_mask = (self.data["train_mask"], self.data["dev_mask"]) if "train_mask" in self.data else (torch.ones_like(self.train_y), torch.ones_like(self.dev_y))
        self.train_dataset = data_utils.TensorDataset(self.train_x, self.train_y, self.train_mask)
        self.dev_dataset = data_utils.TensorDataset(self.dev_x, self.dev_y, self.dev_mask)
    ...
    def _generate_sequence(self, train=True):
        ...
        for sample_batched in data_loader:
            x, y, mask = sample_batched[0].to(self.device), sample_batched[1].to(self.device), sample_batched[2].to(self.device)

            self.exp_state1.append(x)
            self.exp_state1.append(y)
            self.exp_state1.append(mask)
            return
